        ...
        while (!string.IsNullOrEmpty(currentPath))
        {
            using (RegistryKey subKey = rootRegistryKey.OpenSubKey(currentPath))
            {
                if (subKey != null)
                {
                    ...
                    LastWriteTime.Add(DateTime.Now);
...
class Client
{
    private static async Task SendClientLevels(ClientWebSocket webSocket, ClientLevel client)
    {
        ...
        while (webSocket.State == WebSocketState.Open)
        {
            ...
            try
            {
                ...
                if (message == "first run")
                {
                    ...
                    await SendClientLevels(webSocket, clnt);
                }
            }
            catch (JsonException)
            {
